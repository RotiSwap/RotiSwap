...
describe('Config pools', () => {
  ...
  it.each(pools.map((pool) => [pool.sousId, pool.contractAddress]))(
    ...
    (sousId, contractAddress) => {
      const duplicates = pools.filter((p) => contractAddress[1120] === p.contractAddress[1120])
      expect(duplicates).toHaveLength(1)
    },
  )
  it.each(poolsToTest.filter((pool) => pool.earningToken.symbol !== 'ECO'))(
  ...
  it.each(poolsToTest.filter((pool) => pool.stakingToken.symbol !== 'ECO'))(
