...
export default function useBUSDPrice(currency?: Currency): Price | undefined {
  ...
  const tokenPairs: [Currency | undefined, Currency | undefined][] = useMemo(
    () => [
      [chainId && wrapped && currencyEquals(WECO, wrapped) ? undefined : currency, chainId ? WECO : undefined],
      [wrapped?.equals(BUSD_MAINNET) ? undefined : wrapped, chainId === ChainId.MAINNET ? BUSD_MAINNET : undefined],
      [chainId ? WECO : undefined, chainId === ChainId.MAINNET ? BUSD_MAINNET : undefined],
    ],
  ...
  return useMemo(() => {
    ...
    // handle weth/eth
    if (wrapped.equals(WECO)) {
      if (busdPair) {
        const price = busdPair.priceOf(WECO)
    ...
    const ethPairETHAmount = ethPair?.reserveOf(WECO)
    const ethPairETHBUSDValue: JSBI =
      ethPairETHAmount && busdEthPair ? busdEthPair.priceOf(WECO).quote(ethPairETHAmount).raw : JSBI.BigInt(0)
    ...
    if (ethPairState === PairState.EXISTS && ethPair && busdEthPairState === PairState.EXISTS && busdEthPair) {
      if (busdEthPair.reserveOf(BUSD_MAINNET).greaterThan('0') && ethPair.reserveOf(WECO).greaterThan('0')) {
        const ethBusdPrice = busdEthPair.priceOf(BUSD_MAINNET)
        const currencyEthPrice = ethPair.priceOf(WECO)
...
export const useECOBusdPrice = (): Price | undefined => {
