...
const RoiCalculatorFooter: React.FC<RoiCalculatorFooterProps> = ({
  ...
  const {
    ...
    <>
      <Text>
        {t(
          'The Multiplier represents the proportion of ROTI rewards each farm receives, as a proportion of the ROTI produced each block.',
        )}
      </Text>
      <Text my="24px">
        {t('For example, if a 1x farm received 1 ROTI per block, a 40x farm would receive 40 ROTI per block.')}
      </Text>
      <Text>{t('This amount is already included in all ROI calculations for the farm.')}</Text>
    </>,
      ...
      {isExpanded && (
        <Box px="8px">
          <Grid gridTemplateColumns="2.5fr 1fr" gridRowGap="8px" gridTemplateRows={`repeat(${gridRowCount}, auto)`}>
            ...
            <Text color="textSubtle" small>
              {isFarm ? t('Base ROI (ROTI yield only)') : t('ROI')}
            </Text>
            <Text small textAlign="right">
              {apr.toFixed(2)}%
            </Text>
